\o /home/opensrf/scripts/auto_reports/SAGE_monthly_holds2.csv
\a
\f |
select * from
(
...
where
(ahr.request_time >= date_trunc('month', CURRENT_TIMESTAMP - interval '1 month') and
ahr.request_time < date_trunc('month', CURRENT_TIMESTAMP))
...
where
(ahr.shelf_time >= date_trunc('month', CURRENT_TIMESTAMP - interval '1 month') and
ahr.shelf_time < date_trunc('month', CURRENT_TIMESTAMP))
...
join actor.org_unit aou on aou.id=ahr.pickup_lib
where
(ahr.fulfillment_time >= date_trunc('month', CURRENT_TIMESTAMP - interval '1 month') and 
ahr.fulfillment_time < date_trunc('month', CURRENT_TIMESTAMP))
